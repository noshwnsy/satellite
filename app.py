...
# Backup TLE data (Emergency Fallback)
FALLBACK_TLE = """
ISS (ZARYA)
...
def load_satellites():
    ...
    if os.path.exists("active.txt"):
        try:
            ...
            source = "Local File (active.txt)"
        except Exception as e:
            st.warning(f"Found active.txt but failed to read: {e}")
...
with st.spinner("Initializing Data..."):
    ...
    st.success(f"✅ System Online. Data Source: **{source}** ({len(satellites)} satellites)")
...
if df.empty:
    st.error("Data loaded but processing failed.")
    st.stop()
...
if selected_name:
    ...
    # Overlays
    if show_traj: fig.add_trace(go.Scatter3d(x=tx, y=ty, z=tz, mode='lines', line=dict(color='orange', width=4, dash='dot'), name='Orbit Path', hoverinfo='skip'))
    ...
    if is_visible: fig.add_trace(go.Scatter3d(x=[gs_x, row['X']], y=[gs_y, row['Y']], z=[gs_z, row['Z']], mode='lines', line=dict(color='green', width=3), name='Line of Sight', hoverinfo='skip'))
    
    # Nearest
    fig.add_trace(go.Scatter3d(x=[row['X'], nearest_sat['X']], y=[row['Y'], nearest_sat['Y']], z=[row['Z'], nearest_sat['Z']], mode='lines', line=dict(color='magenta', width=2), name=f"Nearest: {nearest_sat['Name']}", hoverinfo='skip'))
